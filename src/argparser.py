...
def parse_args():
    ...
    parser.add_argument("--pipeline_parallel", action="store_true",
                        help="If true, use naive pipeline parallelism, will not give speedup over single-GPU")
    ...
    assert (not (args.model in incompatible_with_generation and \
                 any(['generation' in t for t in
                      args.source_tasks + args.target_tasks]))), f"Cannot perform generation tasks with model {args.model}"
